    ...
    if (!fhevmInstance) {
      ...
      if (!sdk) {
        ...
        if (window.initSDK && window.createInstance) {
          ...
          throw new Error(
            "FHE SDK not found. Available window keys: " +
...
/**
 * 加密多个32位无符号整数
 * @param values - 要加密的数值数组
 * @param contractAddress - 合约地址
 * @param userAddress - 用户地址
 * @returns 加密后的数据和证明
 */
export async function encryptMultipleNumbers(
  values: number[],
  contractAddress: string,
  userAddress: string
  ...
  try {
    ...
    console.log("Creating encrypted input for multiple values:", {
      values,
      contractAddress,
      userAddress,
    });
    ...
    const result = {
      handles: ciphertexts.handles.map((handle: any) =>
        handle instanceof Uint8Array ? toHex(handle) : handle
      ),
      inputProof:
        ciphertexts.inputProof instanceof Uint8Array
    ...
    console.error("Failed to encrypt multiple numbers:", error);
    throw new Error(
      `Failed to encrypt multiple numbers: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
/**
 * 加密64位无符号整数
 * @param value - 要加密的数值 (支持 bigint)
 * @param contractAddress - 合约地址
 * @param userAddress - 用户地址
 * @returns 加密后的数据和证明
 */
export async function encryptNumber64(
  value: number | bigint,
  contractAddress: string,
  userAddress: string
): Promise<{ encryptedData: string; inputProof: string }> {
  try {
    ...
    console.log("Creating encrypted input for 64-bit value:", {
      value,
      contractAddress,
      userAddress,
    });
    ...
    // 执行加密并生成证明
    console.log("Encrypting 64-bit value...");
    const ciphertexts = await buffer.encrypt();
    ...
    const result = {
      encryptedData:
        ciphertexts.handles[0] instanceof Uint8Array
          ? toHex(ciphertexts.handles[0])
    ...
    console.log("Converted 64-bit result:", result);
    return result;
  } catch (error) {
    console.error("Failed to encrypt 64-bit number:", error);
    throw new Error(
      `Failed to encrypt 64-bit number: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
